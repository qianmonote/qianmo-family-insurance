...
- 退出登录需在登录态导航区域提供明确入口：点击后调用 Better-Auth `signOut` 清除 session/cookie，成功后跳转 `/login`，避免用户停留在受保护页面或继续访问个人数据。
...
- 在登录态页面的导航/用户菜单中提供"退出登录"入口。
- 用户点击后调用 Better-Auth `authClient.signOut`，清除当前 session/cookie。
- 退出成功后跳转至 `/login`，并清理受保护页面访问状态；退出失败时展示 toast 错误提示。
- 退出后访问 `/summary`、`/summary/records`、`/summary/records/[id]` 等受保护页面，应继续由全局路由保护重定向到 `/login?redirect=<原路径>`。
