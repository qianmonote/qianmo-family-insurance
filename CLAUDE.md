...
└── <需求主题>/
    ...
    └── design-assets/ # （可选）设计稿还原相关产出：stitch 设计稿截图/导出图、页面截图、还原度对比报告
```

涉及前端页面的需求，在 `design.md` 的"前端页面/组件设计"中为每个页面补充对应的 Figma/Stitch 设计稿链接（字段名约定为 `设计稿`），供"设计稿还原"流程使用；若 PRD 未提供设计稿链接，该页面跳过还原流程，按常规 UI 实现。
...
当最小执行单元是某个页面/组件、且 `design.md` 中该页面标注了"设计稿"（Figma/Stitch 链接）时，在 3.1 实现阶段按以下流程执行：

1. 通过 `stitch` MCP 服务读取该页面的设计稿（截图/结构化描述），作为实现的视觉与结构参照
2. 按设计稿还原页面布局、文案、关键交互元素，同时遵循本文件的前端开发规则（组件拆分、状态管理、表单等）
3. 若 `stitch` MCP 不可用：向用户说明并询问是否使用现有的 `ui_to_artifact` / `ui_diff_check`（zai-mcp-server）等工具替代，或跳过还原校验仅按 PRD 文字描述实现，并在 `lessons.md` 中记录原因

### 单任务循环

每个最小执行单元（任务或子任务）按以下循环执行，全部完成后才进入下一个：

1. **实现**：依据 `design.md` 中对应的设计项，遵循本文件中的前后端开发规则；若涉及前端页面且标注了设计稿，按上方"设计稿还原"流程执行；若实现中发现需调整设计，先更新 `design.md` 再继续；发现粒度过大时先拆分为子任务再继续
2. **自动化测试**：为本次改动补充/更新自动化测试（后端 service/路由的单元或集成测试，前端关键逻辑的单元测试），与实现一并提交；若该最小执行单元确实不适合写测试（如纯静态文案/导航调整），在 `lessons.md` 中说明原因后跳过；涉及设计稿还原的页面，额外按"测试规范"中的"页面还原度验证"补充验证
...
对于按"设计稿还原"流程实现的页面，除常规单元测试外，补充以下验证，结果记录到 `Specs/<主题>/design-assets/` 与 `lessons.md`：

- **视觉还原度**：用 Playwright（或项目已有的 e2e 工具）对实现页面截图，与 `stitch` 设计稿截图通过 `ui_diff_check`（或同类工具）比对，记录明显差异；存在差异时判断是否需要调整实现或是否为可接受的合理差异（如真实数据 vs 设计稿占位数据）
- **结构/文案还原度**：核对关键 DOM 结构、文案、布局元素（导航、表单字段、按钮文案等）与设计稿描述是否一致，可通过简单的 Playwright/Testing Library 断言覆盖关键元素是否存在
- 若该页面无可交互的真实环境（如依赖未配置的第三方服务），还原度验证可降级为"结构/文案"层面，并在 `lessons.md` 中说明原因
