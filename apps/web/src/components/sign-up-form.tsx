...
import { ArrowRight, Eye, EyeOff, Lock, LogIn, Mail, Shield, User, UserPlus } from "lucide-react";
...
export default function SignUpForm() {
  ...
  const loginHref = buildAuthSwitchHref("/login", redirectParam) as Route;
      ...
      <main className="relative z-10 grid w-full max-w-6xl items-center gap-10 xl:grid-cols-[480px_1fr]">
        <section className="rounded-xl border border-[#c3c5d9]/70 bg-white/85 p-8 shadow-2xl backdrop-blur-xl">
          ...
          <header className="mb-6 flex flex-col items-center gap-3 text-center">
            ...
            <div>
              <h1 className="text-3xl font-semibold text-[#0b1c30]">创建账户</h1>
              <p className="mt-2 text-base text-[#434656]">开启您的智能家庭保险管家</p>
            </div>
          </header>
              ...
              {(field) => (
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <User className="absolute left-3 top-1/2 size-5 -translate-y-1/2 text-[#737688]" />
                    <Input
                      id={field.name}
                      name={field.name}
                      type="text"
            ...
            <form.Field name="password">
              {(field) => (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-[#737688]">至少 8 位字符</p>
                  {field.state.meta.errors.map((error) => (
                    <p key={error?.message} className="text-sm text-[#ba1a1a]">
                      {error?.message}
                    </p>
